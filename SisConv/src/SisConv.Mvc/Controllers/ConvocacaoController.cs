...
namespace SisConv.Mvc.Controllers
{
    public class ConvocacaoController : Controller
    {
        ...
        public ActionResult Details(Guid? id)
        {
            ...
            if (convocacaoViewModel.Equals(null))
                return HttpNotFound();
        ...
        public ActionResult Create( ConvocacaoViewModel convocacaoViewModel)
        {
            ...
            var selecionados = convocacaoViewModel.CandidatosSelecionados.Split(',');

            for (int i = 0; i < selecionados.Length; i++)
            {
                convocacaoViewModel.ConvocadoId = Guid.Parse(selecionados[i]);
                convocacaoViewModel.ConvocacaoId = Guid.NewGuid();
                _convocacaoAppService.Add(convocacaoViewModel);
            }

            
            return RedirectToAction("Index", new { @Id = convocacaoViewModel.ProcessoId });
        }
